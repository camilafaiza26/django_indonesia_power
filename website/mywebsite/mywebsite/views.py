		...
		if 'buatgrading' in request.POST:
			...
			for x in range(totalbaris):
				...
				FactGradingRekapanPenilaianPemasok.objects.create(
		            ...
		            p3_total_shipment=total_shipment, p3_jmlh_shipment_tertolak=jumlah_penolakan, p3_persentase_penolakan=persentase_penolakan*100,
...
def printnilai(request):
	if request.user.is_authenticated:
		if request.method == "POST":
			...
			hasil1 = FactGradingRekapanPenilaianPemasok.objects.filter(rekapanpenilaian_id=id)
